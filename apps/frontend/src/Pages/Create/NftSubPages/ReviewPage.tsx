            ...
            <div className={styles.nftPreviewCard}>
                <div className='d-flex gap16'>
                    ...
                    <Col className='d-flex flex-column'>
                        <div className='mb8'>
                            <h2>{details.name}</h2>
                            <div style={{marginLeft: '4px'}} className='lightText12'>{ !isNaN(parseInt(details.tokenId)) ? '#' : 'ID '}{details.tokenId} in collection <span style={{color: '#222'}}>{getCollectionName(collection as Collection)}</span></div>
                        ...
                        {!!details.attributes.filter(a=>!!a.trait_type && !!a.value).length &&<>
                            ...
                            <div className='d-flex flex-wrap gap8 ml8'>
                                {details.attributes.filter(a=>!!a.trait_type && !!a.value).map(a=>
                                    <TraitCard type={a.trait_type} value={a.value} />
                                )}
                            </div>
                        </>}
                    </Col>
